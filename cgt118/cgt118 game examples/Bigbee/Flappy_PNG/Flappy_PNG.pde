...
void setup() { 
  size(600, 800); 
  frameRate(40);
  b=loadImage("Character_jump.png");
  c=loadImage("new_background.png");//this is the background 1800 x 800 Px
  w=loadImage("plant.png");//boundary
  m=loadImage("splash.png");
} 
...
void draw() { //RULES: Max 100 characters per line. 1 semicolon per line, not including for loops
  ...
  if (d==(r=1)) for(imageMode(1);r>0;r--,rectMode(3),text("RecordScore: "+hs,9,780)) image(m, 0, 0);
  
  
  if (mousePressed == true) {
    b=loadImage("Character_jump.png");}
    else{ 
      b=loadImage("Character_idel.png");
