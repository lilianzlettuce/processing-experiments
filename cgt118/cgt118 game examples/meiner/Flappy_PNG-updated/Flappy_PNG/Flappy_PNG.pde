...
void setup() { 
  size(600, 800); 
  frameRate(40);
  b=loadImage("AP&R.png");
  c=loadImage("1800-by-800-BG.png");//this is the background 1800 x 800 Px
  w=loadImage("ROCKET-MAAAAAAAAN!.png");//boundary
  m=loadImage("SplashScreen.png");
} 
...
void draw() { //RULES: Max 100 characters per line. 1 semicolon per line, not including for loops
  ...
  if (d==(r=1)) for(imageMode(1);r>0;r--,rectMode(3),text("High Score: "+hs,9,780)) image(m, 0, 0);
